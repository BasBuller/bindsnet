...
####### Personal functions
def plot_in_out_activity(spikes, figsize=(5, 5)):
    # Input and Ouput layer activitiy
    fig, ax = plt.subplots(figsize=figsize)
    
    in_spikes = spikes["input"].numpy()
    in_rate = in_spikes.sum(1)
    in_rate = in_rate / (in_rate.max() + 1e-6)
    
    out_spikes = spikes["output"].numpy()
    out_rate = out_spikes.sum(1)
    out_rate = out_rate / (out_rate.max() + 1e-6)
    
    ax.plot(in_rate, label="input")
    ax.plot(out_rate, label="output")
    im = ax.legend()
    
    return im


def plot_hidden_activity(spikes, figsize=(5, 5)):
    fig, ax = plt.subplots(figsize=figsize)
    
    spikes = spikes["hidden"].numpy()
    hidden_rate = spikes.sum(1)
    hidden_rate = hidden_rate / (hidden_rate.max() + 1e-6)
    
    ax.plot(hidden_rate, label="hidden")
    im = ax.legend()
