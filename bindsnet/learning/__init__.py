...
class MSTDP(LearningRule):
    ...
    def _connection_update(self, **kwargs) -> None:
        ...
        if not hasattr(self, "eligibility"):
            self.eligibility = torch.zeros_like(self.connection.w)
...
class MSTDPET(LearningRule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.tc_e_trace = torch.tensor(kwargs.get("tc_e_trace", 25.0), device=self.device)
    ...
    def _connection_update(self, **kwargs) -> None:
        ...
        if not hasattr(self, "p_minus"):
            self.p_minus = torch.zeros(self.target.n, device=self.device)
        if not hasattr(self, "eligibility"):
            self.eligibility = torch.zeros_like(self.connection.w)
        if not hasattr(self, "eligibility_trace"):
            self.eligibility_trace = torch.zeros_like(self.connection.w)
    ...
    def _conv2d_connection_update(self, **kwargs) -> None:
        ...
        # Initialize P^+ and P^-.
        if not hasattr(self, "p_plus"):
            self.p_plus = torch.zeros(*self.source.s.size(), device=self.device)
            self.p_plus = im2col_indices(
                self.p_plus, kernel_height, kernel_width, padding=padding, stride=stride
            )
        if not hasattr(self, "p_minus"):
            self.p_minus = torch.zeros(*self.target.s.size(), device=self.device)
            self.p_minus = self.p_minus.view(out_channels, -1).float()
...
class StepMSTDP(LearningRule):
    ...
    def _connection_update(self, **kwargs) -> None:
        ...
        # Parse keyword arguments.
        reward = kwargs["element_wise_reward"]
        a_plus = torch.tensor(kwargs.get("a_plus", 1.0))
        a_minus = torch.tensor(kwargs.get("a_minus", -1.0))
...
class StepMSTDPET(LearningRule):
    # language=rst
    """
    Reward-modulated STDP with eligibility trace. Adapted from
    `(Florian 2007) <https://florian.io/papers/2007_Florian_Modulated_STDP.pdf>`_.
...
class RewardCorrelatedThresholdFedeSTDP(LearningRule):
    def __init__(
        self,
        connection: AbstractConnection,
        nu: Optional[Union[float, Sequence[float]]] = None,
